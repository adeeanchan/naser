<?php
include('header.php');
?>
<header class="page-header" data-background="https://res.cloudinary.com/dcesrrt95/image/upload/v1694383637/valcom/nasser/naser-group/page_bfqnhz.jpg" data-stellar-background-ratio="1.15">
  <div class="container">
    <h1>Flagman</h1>
    <ol class="breadcrumb">
      <li class="breadcrumb-item"><a href="#">Home</a></li>
      <li class="breadcrumb-item active" aria-current="page">Our Communities</li>
    </ol>
  </div>
</header>
<!-- <section class="photo-gallery">
  <div class="container">
    <div class="row">
    	<div class="col-12">
        <ul class="gallery-filter1">
          <li><a href="our-communities.php">Our Portfolio</a></li>
              <li><a href="moore-st-moone.php" >MOORE</a></li>
              <li><a href="buckley-st.php" class="current">BUCKLEY</a></li>
              <li><a href="nicholson-st.php">NICHOLSON</a></li>
              <li><a href="taylor-st.php" >TAYLOR</a></li>
              <li><a href="bewster-st.php" >BREWSTER</a></li>
              <li><a href="pasco-vale.php" >PASCO VALE</a></li>
              <li><a href="clifton-st.php" >CLIFTON</a></li>
              <li><a href="the-boulevard.php" >THE BOULEVARD</a></li>
        </ul>
    	</div>
      <div class="col-12">
        <h2>Buckley St</h2>
        <hr class="line-horiz">
      </div>
    
    <div class="col-12" id="gallery">
      <div class="gallery-container">
      <div class="swiper-wrapper">
          <div class="swiper-slide"><img src="https://res.cloudinary.com/adee-anchan/image/upload/v1696583666/valcom/nasser/naser-group/buckley2_ubd8wr.webp" alt="Image"></div>
          <div class="swiper-slide"><img src="https://res.cloudinary.com/adee-anchan/image/upload/v1696583665/valcom/nasser/naser-group/Buckley1_rbfdud.webp" alt="Image"></div>
         
          
      </div>
      <div class="gallery-pagination"></div>
      </div>
    	</div>
    </div>
  </div>
</section> -->
<section>
  
  <div class="col-12" id="gallery">
      <div class="gallery-container">
      <div class="swiper-wrapper">
          <div class="swiper-slide"><img src="https://res.cloudinary.com/adee-anchan/image/upload/v1707594151/valcom/nasser/naser-group/flagman/2022-Flagman-Portfolio-2000px-05-1920x1008_ilgfdo.png" alt="Image"></div>
          <div class="swiper-slide"><img src="https://res.cloudinary.com/adee-anchan/image/upload/v1707594155/valcom/nasser/naser-group/flagman/2022-Flagman-Portfolio-2000px-03-1920x1008_sng77j.png" alt="Image"></div>
          <div class="swiper-slide"><img src="https://res.cloudinary.com/adee-anchan/image/upload/v1707594155/valcom/nasser/naser-group/flagman/2022-Flagman-Portfolio-2000px-03-1920x1008_sng77j.png" alt="Image"></div>
          <div class="swiper-slide"><img src="https://res.cloudinary.com/adee-anchan/image/upload/v1707594154/valcom/nasser/naser-group/flagman/2022-Flagman-Portfolio-2000px-01-1920x1008_fhhikh.png" alt="Image"></div>
          <div class="swiper-slide"><img src="hhttps://res.cloudinary.com/adee-anchan/image/upload/v1707594154/valcom/nasser/naser-group/flagman/2022-Flagman-Portfolio-2000px-01-1920x1008_fhhikh.png" alt="Image"></div>
          
      </div>
      <div class="gallery-pagination"></div>
      </div>
    	</div>
  </div>
  <div class="container pt-4 pb-5">
    <div class="row">
      <div class="col-md-3">
        <h1 class="text-uppercase">Flagman</h1>
      </div>
      <div class="col-md-5">
        <p>
          The sister development of Porter. Set on beautiful Birmingham street, Flagman uses only carefully selected, timeless materials. Designed to effortlessly contribute to the existing neighbourhood, Flagman carefully curates compositions of form and material to present a timeless facade, softened by the townhouse courtyard landscaping. Exclusive to this stage of Spotswood Yards is 1,875 sqm of amazing lifestyle amenity for residents to enjoy. Featuring entertainment zones for kids, adults, movie buffs, sports fans and creative types, Flagman fulfills every mood and leisure time activity.</p>
      </div>
      <div class="col-md-4">
        <div class="info-com">
          <h5 class="text-uppercase">Location</h5>
          <p>9 BIRMINGHAM STREET
            SPOTSWOOD VICTORIA</p>
        </div>
        <div class="info-com">
          <h5 class="text-uppercase">PROJECT INFORMATION</h5>
          <p>84 APARTMENTS</p>
        </div>
        <div class="info-com">
          <h5 class="text-uppercase">STATUS</h5>
          <p>NOW SELLING. UNDER CONSTRUCTION.</p>
        </div>
      </div>
      
    </div>
  </div>

  

</section>
<?php
include('footer.php');
?>